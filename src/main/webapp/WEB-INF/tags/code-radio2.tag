	...
	for(int i=0; i < codeList.size() ; i++) {
		...
		if( displayCode == null || String.valueOf(code.get("code_value")).matches(displayCode) ){
			...
			<div class="form-check form-check-inline ${inline ? 'radio-inline':''}" ${required ? 'data-parsley-required="true"':'' }>
	            <input class="form-check-input" type="radio" id="<%= name%>_<%=maskNm%>_<%=code.get("code_value")%>" name="<%= name%>" value="<%=code.get("code_value")%>" checked="checked" />
	            <label class="form-check-label" for="<%= name%>_<%=maskNm%>_<%=code.get("code_value")%>"><%=code.get("code_name")%></label>
	        </div>
			<% } else { %>
			<div class="form-check form-check-inline ${inline ? 'radio-inline':''}" ${required ? 'data-parsley-required="true"':'' }>
	            <input class="form-check-input" type="radio" id="<%= name%>_<%=maskNm%>_<%=code.get("code_value")%>" name="<%= name%>" value="<%=code.get("code_value")%>" />
	            <label class="form-check-label" for="<%= name%>_<%=maskNm%>_<%=code.get("code_value")%>"><%=code.get("code_name")%></label>
	        </div>
<% 			}
		}
