							...
							<div class="row">
								<div class="col-md-4 col-12 ">
									<div class="form-group">
										...
										<select class="form-select" name="work_no" id="work_no" data-parsley-required="true" data-parsley-error-message="작업을 선택하세요.">
										</select>
									</div>
								</div>
								...
								<div class="col-4">
									...
									<div class="input-group">
										<select class="form-select" id="use_date" name="use_date"  data-parsley-required="true">
											<option>사용일</option>
										</select>
										...
										<select class="form-select" id="use_hh" name="use_hh" data-parsley-required="true" data-parsley-error-message="사용일(수령일)을 선택하세요">
								...
								<div class="col-md-4 col-12 mt-3">
									...
									<input class="form-control" type="text" id="req_name" name="req_name" value="${data.req_name }" placeholder="100자 이내" data-parsley-required="true" data-parsley-error-message="고객(사)명을 입력하세요." readonly>
								</div>
								<div class="col-md-6 col-12 mt-3">
									<label class="form-label">현장주소<span class="text-danger">*</span></label>
									<input class="form-control" type="text" id="work_addr" name="work_addr" value="${data.work_addr }" placeholder="주소검색" data-parsley-error-message="현장주소를 입력하세요." data-parsley-required="true" readonly>
								</div>
								<div class="col-md-2 col-12 mt-3"></div>
								<div class="col-md-2 col-12 mt-3">
									<label class="form-label">담당자명<span class="text-danger">*</span></label>
									<select class="form-select" id="manager_no" name="manager_no" data-parsley-required="true" data-parsley-error-message="현장주소를 입력하세요.">
									</select>
								</div>
								<div class="col-md-12 col-12 mt-3">
									<label class="form-label">수령방법 <span class="text-danger">*</span></label>
									<tags:code-radio codeList="${receive_type }" checked="${data.receive_type }" name="receive_type" required="true" initValue="01" />
								</div>
							</div>
						</div>
					</div>
...
<div class="d-flex justify-content-center mt-4">
	<button class="btn btn-primary ms-2" id="_dev-close">닫기</button>
	<button class="btn btn-primary ms-2" id="_dev-save">등록</button>
</div>
<!-- begin 자재 목록 -->
<!-- <div class="card  mt-4">
	<div class="card-body"> -->
			...
			<div class="modal-body">
				...
				<form class="form-horizontal form-bordered" data-parsley-validate="true" name="materialFrm" id="materialFrm" novalidate="">
					...
					<!-- <div class="form-row" name="div_add_material" style="display: none">
						<div class="col-md-8 mb-3">
							<label for="material_name_tmp">자재명</label>
							<input type="text" class="form-control" name="material_name" placeholder="자재명" disabled>
						</div>
					</div> -->
						...
						<div class="col-md-3 mb-3">
							...
							<div class="validationTooltip01">
								<select class="form-select" id="unit_cd" name="unit_cd" data-parsley-required="true">
									<option value="">선택</option>
									<tags:code-select codeList="${unit_cd }" selected="${data.unit_cd }" />
								</select>
							</div>
						</div>
						<div class="col-md-3 mb-3">
					...
					<div class="form-row" name="div_add_material" style="display: none">
						...
						<div class="col-md-12 col-sm-8">
							<tags:image-upload name="upLoadFile1" value="${imageList1}" maxFileCount="4" required="false" category="IMG_TYPE_MATERIAL_REQ" type="image" />
						</div>
					</div>

					<div class="form-row mt-3">
						<div class="col-md-3 mb-3">
							<label for="material_cnt">수량</label>
...
<script>
	...
	$(function() {
		...
		$("#_dev-close").on("click", function(e) {
			pageRefresh("/abc/work/view",{"work_no":"${param.work_no}","tab":"#material_view"})
		});
...
//                    contentType: "application/json",
	...
	function changeSelWorkNo(){
		...
		console.log(workNo);
		$("#use_date").empty();
		$("#use_date").append("<option value=\"\">사용일</option>");
	...
	function initMaterialFrom(str, data) {
		...
		if (str === 'ADD') { // 자재추가 form
			...
			$("[name=div_add_material]").show();
			$("[name=div_add_material] input, [name=div_all_material] select")
					.prop("disabled", false);
	...
	function resetMaterialFrom() {
		...
		// 선택폼으로 변경
		$("[name=div_add_material]").hide();
		$("[name=div_add_material] input, [name=div_add_material] select")
				.prop("disabled", true);
...
console.log("{param.work_no}",${param.work_no});
		/* multi form insert */
			...
			success : function(data) {
				...
				if (data != undefined) {
					...
					$.each(data.data, function(index, item) {
						...
						work_list += '<option value="' + item.work_no
								+ '" data-item=\'' + JSON.stringify(temp)
								+ '\' >' + item.work_id + ' | ' + _req_name
		...
		Utils.requestAjax(_name, _url, _param, {
			success : function(data) {
				...
				if (data != undefined) {
					...
					$.each(data.data, function(index, item) {
						let temp = {};
						work_list += '<option value="'+item.admin_no+'" >'
								...
								if (data != undefined) {
									...
									$
											.each(
													data.data,
													function(index, item) {
														temp_list += '<option value="'+ item.category_id +'" data-category_no="'+ item.category_no +'" >'
																+ item.name
																+ '</option>';
													});
		...
		Utils.requestAjax(_name, _url, _param, {
			success : function(data) {
				...
				if (data != undefined) {
					let temp_list = '<option value="">선택</option>';
					$.each(data.data, function(index, item) {
						temp_list += '<option value="'+ item.brand_no +'" >'
			...
			success : function(data) {
				...
				if (data != undefined) {
					...
					$.each(data.data, function(index, item) {
						...
						temp_list += '<option value="' + item.material_no
								+ '" data-item=\'' + JSON.stringify(temp)
								+ '\' >' + _materila_name + '</option>';
