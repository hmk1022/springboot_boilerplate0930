...
<h1 class="page-header">
	구매 <span class="d-block small">자재관리</span>
</h1>
<div class="card">
	<div class="card-header">
		<h3 class="mb-0">구매 등록</h3>
	</div>
	<div class="card-body">
		<form class="form-horizontal form-bordered"
			...
			<div class="row">
				...
				<div class="col-md-12 col-12 mt-3" name="pcs_reason_div_99"
					...
					<input type="text" class="form-control" placeholder="기타사유"
						id="etc_remarks" name="etc_remarks" value="${data.etc_remarks }"
				...
				<div class="col-md-9 col-12 mt-3">
					...
					<div style="margin-top: 3px;">
						<div class="form-check form-check-inline "
							data-parsley-required="true">
							<input class="form-check-input" type="radio" id="vat_yn1"
								name="vat_yn" value="N" ${empty data.vat_yn ||data.vat_yn eq 'N'?'checked':'' }>
							<label class="form-check-label" for="pcs_reason1">부가세별도</label>
						</div>
						<div class="form-check form-check-inline "
							data-parsley-required="true">
							<input class="form-check-input" type="radio" id="vat_yn2"
								name="vat_yn" value="Y" ${data.vat_yn eq 'Y'?'checked':'' }>
...
<c:if test="${empty data.material_pcs_no }">
    <button class="btn btn-gray" name="_button_page_list" data-to_url="/material/pcs/list">list</button>
 </c:if>
 <c:if test="${!empty data.material_pcs_no }">
    <button class="btn btn-gray" name="_button_page_view" data-to_url="/material/pcs/view" data-to_param="material_pcs_no^${data.material_pcs_no}">cancel</button>
</c:if>
				...
				<form class="form-horizontal form-bordered"
					...
					<div class="form-row mt-3">
						<div class="col-md-3 mb-3">
							...
							<input type="text" class="form-control" id="item_cnt" name="item_cnt" value="0" placeholder="수량"
								min="0" max="10000" 
								data-parsley-required="true"
						      	data-parsley-min="1"
						...
						<div class="col-md-3 mb-3">
							...
							<input type="text" 
								class="form-control" 
								id="total_material_price"
								name="total_material_price" 
								value="0" placeholder="금액" 
								readonly>
						</div>
					</div>
	...
	$(function(){
		...
		$("#_dev-save").on("click", function(e){
			...
			Utils.requestMultiAjax(_name, _url, _form, {
	            success:  function(data) {
		          	...
		          	//listMaterialPcsItem();
		          	let param = {
						material_pcs_no : $("[name=material_pcs_no]").val()
					}
	    ...
	    $("[name=pcs_reason]").on("change", function(){
	    	...
	    	if($(this).val() != '01'){
	    		...
	    		$("#material_doc_no").attr("data-parsley-required","false");
	    	}
	    });
            ...
            success:  function(data) {
	          	...
	          	if(Utils.isEmpty(data.data)){
	          		...
	          		$.each(data.data, function(idx, item){
	    				...
	    				'<td style="text-align:right">'+Utils.comma(Utils.nvl(item.total_purchased_price))+'</td>'+
