...
<h1 class="page-header">
	비용처리 <span class="d-block small">회계관리</span>
</h1>
<!-- end page-header -->
<!-- begin panel -->
<div class="card">
	...
	<div class="card-body" style="">
		<form class="form-horizontal form-bordered"
			data-parsley-validate="true" name="saveFrm" id="saveFrm"
			novalidate="" enctype="multipart/form-data">
			<input type="hidden" id="exp_no" name="exp_no"
				value="${data.exp_no }"> <input type="hidden" id="bank_code"
				name="bank_code" value="${data.bank_code }"> <input
				type="hidden" id="acct" name="acct" value="${data.acct }">

			<div class="row">

				<div class="col-md-6 col-12 mt-3 ">
					<label class="form-label">작업선택 <span class="text-danger">*</span></label>
					<select class="form-select" name="work_no" id="work_no"
						data-parsley-required="true"
						data-parsley-error-message="작업선택하세요.">
					</select>
				</div>
				<div class="col-md-3 col-12 mt-3 ">
					...
					<select class="form-select" id="exp_type" name="exp_type"
						data-parsley-required="true"
						data-parsley-error-message="비용분류를 선택하세요.">
						<option value="">선택</option>
						<tags:code-select codeList="${exp_type }"
							selected="${data.exp_type }" />
					</select>
				</div>
				<div class="col-md-3 col-12 mt-3">
					<label class="form-label">품의번호 <span class="text-danger">*</span></label>
					<input type="text" class="form-control " placeholder="비용용도"
						name="exp_id" id="exp_id" value="${data.exp_id }" readonly
						data-parsley-error-message="품의번호을 정확히 입력하세요.">
				</div>
				<div class="col-md-12 col-12 mt-3">
					<label class="form-label">비용용도 <span class="text-danger">*</span></label>
					<input type="text" class="form-control " placeholder="비용용도"
						name="exp_usage" id="exp_usage" value="${data.exp_usage }"
						data-parsley-required="true"
						data-parsley-error-message="비용용도을 정확히 입력하세요.">
				</div>

				<div class="col-md-4 col-12 mt-3 " name="exp_type_01">
					<label class="form-label">외주용역선택 <span class="text-danger">*</span></label>
					<select class="form-select" id="partner_no" name="partner_no"
						data-parsley-required="true"
						data-parsley-error-message="외주용역을 선택하세요.">
						<option value="">선택</option>
						<c:forEach var="item" items="${partner_list }">
							<option value="${item.partner_no }"
								data-partner_hp1="${item.partner_hp1}"
								...
								${data.partner_no eq item.partner_no?'selected':'' }>${item.partner_name }</option>
						</c:forEach>
					</select>
				</div>

				<div class="col-md-4 col-12 mt-3" name="exp_type_03"
					style="display: none">
					<label class="form-label">매입거래처<span class="text-danger">*</span></label>
					<select class="form-select" id="vendor_no" name="vendor_no"
						data-parsley-required="true">
						<option value=''>선택</option>
				...
				<div class="col-md-4 col-12 mt-3 ">
					...
					<input type="text" class="form-control " placeholder="결제정보"
						name="pay_info" id="pay_info" value="${data.pay_card_no }" data-parsley-required="true"
						data-parsley-error-message="경제정보를 정확히 입력하세요."
						readonly>
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					<label class="form-label">지출증빙 <span class="text-danger"></span></label>
					<input type="text" class="form-control " placeholder="지출증빙"
						name="evidence" id="evidence" value="${data.evidence }" readonly>
				</div>

				<div class="col-md-4 col-12 mt-3 " name="exp_type_04"
					style="display: none">
					<label class="form-label">사용처 <span class="text-danger">*</span></label>
					<input type="text" class="form-control " placeholder="사용처"
						data-parsley-required="true" data-parsley-error-message="사용처를 정확히 입력하세요"
						name="exp_target" id="exp_target" value="${data.exp_target }">
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					<label class="form-label">사용일 <span
						class="text-danger">*</span></label> <input type="text"
						class="form-control " placeholder="사용일(시작일)" name="st_date"
						id="st_date" value="${data.st_date }" data-parsley-required="true"
						data-parsley-error-message="사용일(시작일)을 선택하세요." datepicker>
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					<label class="form-label">결제요청일 <span class="text-danger">*</span></label>
					<input type="text" class="form-control " placeholder="결제요청일 "
						name="pay_req_date" id="pay_req_date"
						data-parsley-required="true"
						data-parsley-error-message="결제요청일을 선택하세요"
						value="${data.pay_req_date }" datepicker>
				</div>				
				<div class="col-md-4 col-12 mt-3 "></div>
				<div class="col-4 mt-3 mt-3 ">
					<label class="form-label">비용단위 <span class="text-danger"></span></label>
					<select class="form-select" id="exp_unit_cd" name="exp_unit_cd"
						data-parsley-required="true"
						data-parsley-error-message="비용단위를 정확히 입력하세요.">
						<option value="">선택</option>
						<tags:code-select codeList="${exp_unit_cd }"
							selected="${data.exp_unit_cd }" />
					</select>
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					<label class="form-label">갯수 <span class="text-danger">*</span></label>
					<input type="text" class="form-control " placeholder="갯수"
						name="exp_cnt" id="exp_cnt" value="${data.exp_cnt }"
						data-parsley-required="true" data-parsley-type="number"
						data-parsley-error-message="갯수를 정확히 입력하세요.">
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					<label class="form-label">단가 <span class="text-danger">*</span></label>
					<input type="text" class="form-control " placeholder="단가"
						name="exp_price" id="exp_price" value="${data.exp_price }"
						data-parsley-required="true"
						data-parsley-error-message="단가를 정확히 입력하세요.">
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					...
					<div style="margin-top: 3px;">
						<div style="margin-top: 3px;">
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio" id="pay_type1"
									name="pay_type" 
									value='ACCT'
									checked> <label
									class="form-check-label" for="pay_type1" >계좌이체</label>
							</div>
						</div>
					</div>
				</div>
				<div class="col-md-4 col-12 mt-3 ">
					...
					<input type="text" class="form-control " placeholder="단가"
						id="total_exp_price" value="${data.exp_cnt * data.exp_price }"
						readonly comma
						>
				</div>					
				<div class="col-md-12 col-12 mt-3 ">
					...
					<tags:image-upload name="upLoadFile" value="${imageList}"
						maxFileCount="10" required="true" category="IMG_TYPE_EXPENSES"
						type="image" />
				</div>
