...
<%@ include file="/WEB-INF/commons/taglibs.jsp"%>
	...
	<!-- begin page-header -->
	<h1 class="page-header">workerman 계정 관리 <span><small>workerman 운영<small></<span></h1>
	<!-- end page-header -->
	<!-- begin panel -->
	<div class="card">
		<div class="card-header">
			<h4 class="panel-title">workerman 계정 등록</h4>
		</div>
		<div class="card-body" style="">
			<form class="form-horizontal form-bordered" data-parsley-validate="true" name="saveForm" id="frm" novalidate="" enctype="multipart/form-data">
			...
			<div class=" row ">
			<div class="col-md-6 col-12 mt-3 custom-type-p" id="display_none" >
			...
			<div class="col-md-6 col-12 mt-3 custom-type-p">
				    ...
				    <select name="company_no" class="form-control" placeholder="지점명" data-parsley-required="true" data-parsley-id="8" aria-describedby="parsley-id-8" data-parsley-error-message="업체를 선택하세요">
						<tags:code-select
							codeList="${company }"
			...
			<div class="col-md-6 col-12 mt-3 custom-type-p" >
				<label class="form-label" for="main_type">워커맨팀</label>
				<%-- <input class="form-control " type="text" id="department_cd" name="department_cd" value="${data.group_name }" placeholder="부서" data-parsley-required="true" data-parsley-id="4" aria-describedby="parsley-id-4" data-parsley-error-message="부서를 정확히 입력하세요"> --%>
				<select class="form-select" id="main_type" name="main_type" data-parsley-required="false" data-parsley-id="" data-parsley-error-message="부서를 선택하세요">
						  <option value="${data.main_type}" class="main_type" selected>선택</option>
			...
			<%-- <div class="col-md-6 col-12 mt-3 custom-type-p">
					...
					<option value="${data.stat_cd}">계정상태</option>
						<c:forEach var="item" items="${stat_cd}">
			...
			<div class="col-md-6 col-12 mt-3 custom-type-p" >
				<label class="form-label" for="req_name">연락처</label>
				<input class="form-control " type="text" id="mobile" name="mobile" value="${data.mobile }" placeholder="연락처" data-parsley-required="false" data-parsley-id="4" aria-describedby="parsley-id-4" data-parsley-error-message="전화번호를 정확히 입력하세요" >
			</div>
			<div class="col-md-6 col-12 mt-3 custom-type-p">
					<label class="form-label" for="level_cd">숙련도</label>
				    <select id="level_cd" name="level_cd" class="form-control " placeholder="계정상태" data-parsley-required="false" data-parsley-id="9" aria-describedby="parsley-id-9" data-parsley-error-message="계정상태를 선택하세요">
						<option class= "level" value="${data.level_cd }" selected>${data.level_name }</option>
		 	...
		 	<div class="col-12 mt-3 row " id="admin_cates">
				<label class="col-md-2 col-sm-4 col-form-label" >전문분야</label>
						<div class="checkbox d-flex" >
							<input type="hidden" name="admin_cates" id="admin_cates">
							<c:forEach var="data" items="${admin_cate}" varStatus="cnt">
								<input id="admin_cate_${cnt.count}" name="admin_cate_${cnt.count}" style="width: 15px; height:15px; margin: 0 3px;"
									type="checkbox" value="${data.code_value}">
								<label for="admin_cate_${cnt.count}" class="mr10" style="font-size: 11px">
				...
				<div class="col-sm-14 text-center">
					<button type="button" class="btn btn-primary" id="_dev-save">save</button>
						<c:if test="${empty data.admin_no }">
						    <button class="btn btn-gray" name="_button_page_list" data-to_url="/operation/workerman/index">list</button>
						 </c:if>
						 <c:if test="${!empty data.admin_no }">
						    <button class="btn btn-gray" name="_button_page_view" data-to_url="/operation/admin/wview" data-to_param="admin_no^${data.admin_no}">cancel</button>
						</c:if>
				</div>
			</div>
			</form>
...
<script>
	...
	$(document).ready(function() {
		...
		if(!data){
			...
			//$('#admin_cates').css("display","none");
			$('#admin_id').attr('disabled', true);
			$('#password').attr('data-parsley-required',false);
			
		}
	
		console.log('워커맨팀','${data.main_type}')
		
		if('${data.main_type}'== '00') {
			$('.main_type').val('00');
			$('.main_type').text('견습');
		} else if ('${data.main_type}'== '01'){
			$('.main_type').val('01');
			$('.main_type').text('B2C');
		} else if ('${data.main_type}'== '02'){
			$('.main_type').val('02');
			$('.main_type').text('프로젝트팀');
		} else if ('${data.main_type}'== '03'){
			$('.main_type').val('03');
			$('.main_type').text('B2B');
		}
...
<script>
	...
	$(function(){
		...
		let specialData = '${special_cd}';
		let parseData = JSON.parse(specialData);
		let special_arr = [];
		for (let i = 0; i < parseData.length; i++){
			special_arr.push(parseData[i].special_cd);
		};
		
		
		/* 전문분야 체크 */
		for(let j = 0; j < special_arr.length; j++ ){	
			for (let i = 1; i < 17; i++){			
				if(special_arr[j] == $('#'+ 'admin_cate_' + i ).val()){
					$('#'+ 'admin_cate_' + i ).attr('checked', true);	
				}	
			};
		}
	

		console.log('special',special_arr);
		
		let data = '${data}'
		
		/* if(data) {
			$('.main_tp').val('${data}.main_type');
			$('.main_tp').text('${data}.main_name');
		} */
		if (data){
			$('#admin_id').attr('disabled', true);
		...
		$("#_dev-save").on("click", function(e){
			...
			const _url = "/operation/admin/wsave/ajax";	// ajax url
